...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderCarCard_3js(car, cardPurpose = 'general') {
        ...
        if (!car || !car.ID) {
            console.error("renderCarCard_3js nhận dữ liệu xe không hợp lệ:", car);
            carBox.innerHTML = "<p>Lỗi hiển thị xe.</p>";
            return carBox;
        }
        ...
        if (window.HuanHungCarshopApp && typeof window.HuanHungCarshopApp.isCarSaved === 'function' && window.HuanHungCarshopApp.isCarSaved(car.ID)) {
            heartIcon.classList.add('bxs-heart', 'saved');
            heartIcon.title = "Bỏ lưu xe này";
        } else {
            heartIcon.classList.add('bx-heart');
            heartIcon.title = "Lưu xe này";
        }
            ...
            if (window.HuanHungCarshopApp && typeof window.HuanHungCarshopApp.toggleSaveCar === 'function') {
                ...
                if (result.success && !result.isNowSaved && cardPurpose === 'saved') {
                     ...
                     if (savedCarsContainer && savedCarsContainer.children.length === 0) {
                         savedCarsContainer.innerHTML = `<p class="no-cars-message">Bạn chưa lưu xe nào. Hãy <a href="#home" class="text-link" onclick="resetFilterView_3js(document.querySelector('.navbar a.nav-link[href=\\'#home\\']'));">khám phá xe</a> và lưu lại!</p>`;
                     }
                }
            } else {
                alert("Chức năng lưu xe chưa sẵn sàng hoặc bạn cần đăng nhập.");
            }
        });

        let imageHTML = `<img src="${firstImage}" alt="${car.brand || ''} ${car.model || ''}">`;
        let priceHTML = '';
        if (styleType === 'new' && cardPurpose !== 'user-posted' && cardPurpose !== 'saved') {
            priceHTML = `<p class="car-card-price">${car.price ? car.price.toLocaleString('vi-VN') + ' VNĐ' : 'Liên hệ'}</p>`;
        }
        ...
        if (cardPurpose === 'user-posted') {
            footerHTML = `
                <div class="box-footer-actions">
                    <a href="#" class="details-btn action-btn-card">Xem Chi Tiết</a>
                    <a href="#" class="edit-btn action-btn-card">Sửa Tin</a>
                </div>`;
        ...
        carBox.innerHTML = imageHTML + priceHTML + textContentHTML + footerHTML;
        carBox.insertBefore(heartIcon, carBox.firstChild);
        ...
        if (cardPurpose === 'user-posted') {
            ...
            if (editBtn) {
                editBtn.addEventListener('click', (e) => {
                    e.preventDefault();
                    e.stopPropagation();
                    const carIdToEdit = carBox.getAttribute('data-car-id');
                    window.location.href = `./salespage.html?editCarId=${carIdToEdit}`;
                });
            }
        }
        carBox.addEventListener('click', function(e) {
            if (!e.target.closest('.car-card-heart') && !e.target.closest('.action-btn-card')) {
    ...
    function displayUserPostedCars_3js() {
        if (!userCarsContainer || !userPostedCarsSection) {
            return;
        }
        userCarsContainer.innerHTML = '';
        const loggedInUserId = localStorage.getItem('loggedInUserId');

        if (!loggedInUserId) {
            userCarsContainer.innerHTML = `<p class="no-cars-message">Vui lòng <a href="login.html" class="text-link">đăng nhập</a> để xem xe của bạn.</p>`;
        ...
        if (userData && userData.carAds && userData.carAds.length > 0) {
            ...
            sortedCarAds.forEach(car => {
                userCarsContainer.appendChild(renderCarCard_3js(car, 'user-posted'));
            });
            userPostedCarsSection.style.display = 'block';
        } else {
            userCarsContainer.innerHTML = `<p class="no-cars-message">Bạn chưa đăng xe nào. <a href="salespage.html" class="text-link">Đăng ngay!</a></p>`;
            userPostedCarsSection.style.display = 'block';
        }
    }
    ...
    window.displaySavedCarsFrom3JS = function() {
        if (!displaySection || !displayContainer || !categorySpan || !titleH2 || !descriptionPFiltered) {
            console.error("Một hoặc nhiều phần tử hiển thị không tìm thấy cho 'Tin đã lưu'.");
            return;
        }
        ...
        setActiveNavLink_3js(null);

        const loggedInUserId = localStorage.getItem('loggedInUserId');
        if (!loggedInUserId) {
            categorySpan.textContent = "Danh sách yêu thích";
            titleH2.textContent = "Xe bạn đã lưu";
            descriptionPFiltered.textContent = "Xem lại những chiếc xe bạn quan tâm.";
            displayContainer.innerHTML = `<p class="no-cars-message">Vui lòng <a href="login.html" class="text-link">đăng nhập</a> để xem xe đã lưu.</p>`;
            displaySection.scrollIntoView({ behavior: 'smooth', block: 'start' });
            return;
        }

        const savedCarIds = (window.HuanHungCarshopApp && typeof window.HuanHungCarshopApp.getSavedCarsForCurrentUser === 'function')
                            ? window.HuanHungCarshopApp.getSavedCarsForCurrentUser() : [];

        const userSpecificData = JSON.parse(localStorage.getItem('userSpecificData')) || {};
        let allCarsMasterList = [];
        for (const userId in userSpecificData) {
            if (userSpecificData[userId]?.carAds) {
                userSpecificData[userId].carAds.forEach(carAd => allCarsMasterList.push({ ...carAd, userId }));
            }
        }

        const savedCarsDetails = savedCarIds.map(savedId => {
            return allCarsMasterList.find(car => car.ID === savedId);
        ...
        if (savedCarsDetails.length > 0) {
            savedCarsDetails.forEach(car => {
                displayContainer.appendChild(renderCarCard_3js(car, 'saved'));
            });
        } else {
            displayContainer.innerHTML = `<p class="no-cars-message">Bạn chưa lưu xe nào. Hãy <a href="#home" class="text-link" onclick="resetFilterView_3js(document.querySelector('.navbar a.nav-link[href=\\'#home\\']'));">khám phá xe</a> và lưu lại!</p>`;
        }
        displaySection.scrollIntoView({ behavior: 'smooth', block: 'start' });
    };

    function setActiveNavLink_3js(activeLink) {
    ...
    function displayFilteredResults_3js(filterType, filterValue = null, source = 'navbar') {
        if (!displaySection || !displayContainer || !categorySpan || !titleH2 || !descriptionPFiltered) {
            console.error("Một hoặc nhiều phần tử hiển thị không tìm thấy cho 3.js filtering.");
            return;
        }
        ...
        let titleText = "";
        let descriptionText = "Hiển thị xe dựa trên tiêu chí của bạn.";

        if (filterType === 'condition') {
            filteredCars = allCars.filter(car => car.condition === filterValue);
            categoryText = filterValue === 'new' ? `Xe Mới (${source === 'navbar' ? 'Lọc Nhanh' : 'Khác'})` : `Xe Cũ (${source === 'navbar' ? 'Lọc Nhanh' : 'Khác'})`;
            titleText = filterValue === 'new' ? "Danh sách xe mới" : "Danh sách xe đã qua sử dụng";
            descriptionText = filterValue === 'new' ? `Khám phá các mẫu xe mới nhất.` : `Tìm các xe đã qua sử dụng chất lượng.`;
        } else if (filterType === 'seats') {
            let seatDescription = "";
            if (filterValue === "8+") {
                filteredCars = allCars.filter(car => car.seats && (parseInt(car.seats) >= 8 || car.seats.toLowerCase().includes("8+")));
                ...
                if (!isNaN(seatNumber)) {
                    ...
                    filteredCars = []; seatDescription = "không hợp lệ";
                }
            }
            categoryText = "Lọc theo số chỗ";
            titleText = `Xe ${seatDescription}`;
            descriptionText = `Hiển thị xe có ${seatDescription}.`;
            ...
            categoryText = "Kết quả tìm kiếm";
            titleText = `Xe cho từ khóa: "${filterValue}"`;
            descriptionText = `Đã tìm thấy ${filteredCars.length} xe.`;
        } else {
            console.error("Unknown filter type in 3.js:", filterType);
            return;
        }
        ...
        descriptionPFiltered.textContent = descriptionText;

        if (filteredCars.length > 0) {
            filteredCars.forEach(car => {
                displayContainer.appendChild(renderCarCard_3js(car, 'filtered'));
            });
        } else {
            displayContainer.innerHTML = `<p class="no-cars-message">Không tìm thấy xe nào phù hợp với tiêu chí của bạn.</p>`;
        }
        document.querySelectorAll('.default-view-section').forEach(sec => {
            if (sec) sec.style.display = 'none';
        });
         ...
         document.querySelectorAll('.default-view-section').forEach(sec => {
              if(sec) {
                  if (sec.id === 'user-posted-cars') {
                      ...
                      sec.style.display = sec.id === 'about' ? 'flex' : 'block';
                  }
              }
         });
    ...
    function loadInitialCarsIntoSections_3js() {
        ...
        for (const userId in userSpecificData) {
            if (userSpecificData[userId]?.carAds) {
                userSpecificData[userId].carAds.forEach(carAd => allCars.push({ ...carAd, userId }));
            }
        }

        if (newCarsContainer) {
            newCarsContainer.innerHTML = '';
            const newCarsData = allCars.filter(car => car.condition === 'new').sort((a,b) => new Date(b.postedDate) - new Date(a.postedDate));
            if (newCarsData.length > 0) {
                newCarsData.slice(0, 6).forEach(car => newCarsContainer.appendChild(renderCarCard_3js(car, 'new-onload')));
            } else {
                newCarsContainer.innerHTML = `<p class="no-cars-message">Hiện chưa có xe mới nào được đăng.</p>`;
            }
        }
        if (oldCarsContainer) {
            oldCarsContainer.innerHTML = '';
            const oldCarsData = allCars.filter(car => car.condition === 'used').sort((a,b) => new Date(b.postedDate) - new Date(a.postedDate));
            if (oldCarsData.length > 0) {
                oldCarsData.slice(0, 6).forEach(car => oldCarsContainer.appendChild(renderCarCard_3js(car, 'old-onload')));
            } else {
                oldCarsContainer.innerHTML = `<p class="no-cars-message">Hiện chưa có xe cũ nào được đăng.</p>`;
            }
        }
    }
    loadInitialCarsIntoSections_3js();
    ...
    if (searchInputGlobal_3js) {
        let searchTimeout_3js;
        searchInputGlobal_3js.addEventListener('input', () => {
            clearTimeout(searchTimeout_3js);
            const searchTerm = searchInputGlobal_3js.value.trim();
            if (searchTerm === "") {
                resetFilterView_3js(document.querySelector('.navbar a.nav-link[href="#home"]'));
            } else {
                searchTimeout_3js = setTimeout(() => {
                    displayFilteredResults_3js('search', searchTerm);
                    setActiveNavLink_3js(null);
                }, 500);
            }
        });
        searchInputGlobal_3js.addEventListener('keypress', (e) => {
            if (e.key === 'Enter') {
                e.preventDefault();
                clearTimeout(searchTimeout_3js);
                const searchTerm = searchInputGlobal_3js.value.trim();
    ...
    if(window.location.hash === '#user-posted-cars') {
        const userPostedLink = document.querySelector('.navbar a.nav-link[href="#user-posted-cars"]');
        resetFilterView_3js(userPostedLink);
        setTimeout(() => {
            const targetSection = document.getElementById('user-posted-cars');
            if (targetSection) {
        ...
        const sectionToScroll = document.getElementById(targetIdFromHash);
        if(sectionToScroll && sectionToScroll.classList.contains('default-view-section')){
             const correspondingNavLink = document.querySelector(`.navbar a.nav-link[href="#${targetIdFromHash}"]`);
             resetFilterView_3js(correspondingNavLink);
             setTimeout(() => {
                sectionToScroll.scrollIntoView({ behavior: 'smooth', block: 'start' });
             },100);
        }
    }
    console.log("3.js (no cart logic, includes UserPostedCars logic) loaded.");
});